...
function MapPanel({ place, mapsLoaded, liveData }) {
  return <section className="map-panel" aria-label="Live risk map"><div className="map-heading"><div><span className="section-kicker">REGION 07 / CENTRAL INDIA</span><h2>Hyper-local flood intelligence</h2></div><span className="map-status"><i /> {mapsLoaded ? 'Google satellite feed' : 'Google Maps setup required'}</span></div><div className="empty-map">{mapsLoaded ? <GoogleMapView place={place} liveData={liveData} /> : <div className="map-setup"><Map size={25} /><strong>Google Maps is ready to connect</strong><span>Set VITE_GOOGLE_MAPS_API_KEY to enable Places, satellite, terrain, road, hybrid, and flood overlays.</span></div>}</div><div className="map-footer"><span><i className="legend-dot critical" /> Critical</span><span><i className="legend-dot high" /> High</span><span><i className="legend-dot moderate" /> Watch</span><span className="coordinates">3 monitored zones / 12 active sensors</span></div></section>
}

function GoogleMapView({ place, liveData }) {
  ...
  const openInfoWindow = useCallback((map) => {
    ...
    infoWindowRef.current = new google.maps.InfoWindow({ content, ariaLabel: place.name })
