...
function Topbar({ mapsLoaded, onPlaceSelected }) {
  ...
  const search = <div className="search-box"><Search size={16} />{mapsLoaded ? <input aria-label="Search locations" placeholder="Search locations" onKeyDown={(event) => { if (event.key === 'Enter') event.preventDefault() }} /> : <span>{import.meta.env.VITE_GOOGLE_MAPS_API_KEY ? 'Loading Google Maps' : 'Add Google Maps key'}</span>}<kbd>/</kbd></div>
