...
function MissionCard({ place, prediction, alert, onOpenResponsePlan }) {
  ...
  const handleOpenResponsePlan = () => {
    onOpenResponsePlan?.()
  }

  return (
    <section className="mission-card glass-panel">
      <div className="mission-top">
        <span className="section-kicker">CURRENT MISSION</span>
        <span className={`mission-id ${riskClass}`}>#NDRF-071 / {riskLevel}</span>
      </div>
      <h2>{title}</h2>
      <p>{missionCopy}</p>
      ...
      <div className="mission-stats">
        <span><LocateFixed size={14} /><strong>03</strong> zones</span>
        <span><Clock3 size={14} /><strong>{leadTime}</strong> min lead</span>
        <span><ShieldAlert size={14} /><strong>{riskScore}</strong> score</span>
      </div>
      <button type="button" className="mission-action" onClick={handleOpenResponsePlan}>
        Open response plan <ArrowUpRight size={15} />
      </button>
    </section>
  )
}
